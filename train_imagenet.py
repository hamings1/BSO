...
parser.add_argument('-T', '--time_step', type=int, default=4, metavar='time',
...
parser.add_argument('-data-dir', metavar='DIR',default="/data/dataset/ImageNet",
...
parser.add_argument('--gamma',default=1e-3,type=float,metavar='N')
parser.add_argument('--beta',default=1e-5,type=float,metavar='N')
...
def main():
    setup_default_logging(log_path='./train2.log')
    args, args_text = _parse_args()
    _logger.info('Training FULL Precision')
    if args.log_wandb:
        if has_wandb:
    ...
    model = create_model(
        ...
        tau=args.tau, surrogate_function=surrogate.Sigmoid(alpha=4.), 
        track_rate=True, c_in=3, num_classes=num_classes, 
        drop_rate=args.drop_rate, stochdepth_rate=args.stochdepth_rate, 
        neuron_dropout=0.0, grad_with_rate=True, v_reset=None
        )
    ...
    if args.optimizer == 'SGD':
        ...
        print("Use bop optmizer")
        optimizer = Bop(model,lr=args.lr,threshold=args.threshold, gamma=args.gamma,weight_decay=args.weight_decay)
        gamma_sch = CustomScheduler(optimizer, param_name="gamma", decay_epochs=100, decay=0.1)
    ...
    # loader_train = DataLoader(trainset, batch_size=args.batch_size, shuffle=True, num_workers=args.workers, pin_memory=True)
    # loader_eval = DataLoader(testset, batch_size=args.batch_size, shuffle=False, num_workers=args.workers, pin_memory=True)
        ...
        with amp_autocast():
            ...
            for t in range(args.time_step):
                ...
                if args.loss_lambda > 0.0:
                    label_one_hot = F.one_hot(target, 1000).float()
                    mse_loss = nn.MSELoss(out_fr, label_one_hot)
                    loss = ((1 - args.loss_lambda) * F.cross_entropy(out_fr, target) + args.loss_lambda * mse_loss) / args.time_step
                else:
                    loss = F.cross_entropy(out_fr, target) / args.time_step
            ...
            if not args.online_update:
                if args.clip_grad is not None:
                    dispatch_clip_grad(
                        model_parameters(model, exclude_head='agc' in args.clip_mode),
                        value=args.clip_grad, mode=args.clip_mode)
                optimizer.step(args.time_step,args.time_step)
